...
const (
	cmdDataPartitionGetShort          = "Show information of data partition"
	cmdCheckCorruptDataPartitionShort = "Check corrupt data partitions"
	cmdResetDataPartitionShort        = "Check corrupt data partitions"
)
...
func newListCorruptDataPartitionCmd(client *master.MasterClient) *cobra.Command {
	var cmd = &cobra.Command{
		...
		Long: `If the data nodes are marked as "Inactive", it means the nodes has been not available for a time. It is suggested to eliminate
				...
				the corrupt nodes, the few remaining replicas can not reach an agreement with one leader. In this case, you can use the "reset"
				command to fix the problem, however this action may lead to data loss, be careful to do this.`,
		Run: func(cmd *cobra.Command, args []string) {
			var (
