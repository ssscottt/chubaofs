...
func (m *Server) decommissionDataPartition(w http.ResponseWriter, r *http.Request) {
	...
	if err = m.cluster.decommissionDataPartition(addr, dp, getTargetAddressForDataPartitionDecommission, handleDataPartitionOfflineErr); err != nil {
		sendErrReply(w, r, newErrHTTPReply(err))
		return
	}
...
func (m *Server) decommissionMetaPartition(w http.ResponseWriter, r *http.Request) {
	...
	if err = m.cluster.decommissionMetaPartition(nodeAddr, mp, getTargetAddressForMetaPartitionDecommission); err != nil {
		sendErrReply(w, r, newErrHTTPReply(proto.ErrMetaPartitionNotExists))
	}else {
...
func parseAndExtractSetNodeInfoParams(r *http.Request) (params map[string]interface{}, err error) {
	...
	if noParams, err = parseNodeInfoKey(params, nodeAutoRepairRateKey, noParams, r); err != nil {
		return
	}
	if noParams, err = parseNodeInfoKey(params, nodeDeleteWorkerSleepMs, noParams, r); err != nil {
		return
	}
	if noParams, err = parseNodeInfoKey(params, dpRecoverPoolSizeKey, noParams, r); err != nil {
		return
	}
	if noParams, err = parseNodeInfoKey(params, mpRecoverPoolSizeKey, noParams, r); err != nil {
		return
	}
	if noParams {
