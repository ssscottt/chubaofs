...
func (c *Cluster) repairDataPartition(){
	...
	go func() {
		for {
			if uint64(c.dataPartitionInRecovering()) > c.cfg.dataPartitionsRecoverPoolSize {
				log.LogWarnf("action[repairDataPartition] clusterID[%v] Recover pool is full", c.Name)
			...
			case task := <-c.dpRepairChan:
				...
				case BalanceDataZone:
					if err = c.decommissionDataPartition("", dp, getTargetAddressForRepairDataZone, "BalanceDataZone"); err != nil {
						goto errorHandler
					}
				default:
...
func (c *Cluster) repairMetaPartition(){
	...
	go func() {
		for {
			if uint64(c.metaPartitionInRecovering()) > c.cfg.metaPartitionsRecoverPoolSize {
				log.LogWarnf("action[repairMetaPartition] clusterID[%v]Recover pool is full", c.Name)
				time.Sleep(10 * time.Second)
				continue
			}
			select {
			case task := <-c.mpRepairChan:
				...
				case BalanceMetaZone:
					if err = c.decommissionMetaPartition("", mp, getTargetAddressForRepairMetaZone); err != nil {
						goto errorHandler
					}
				default:
...
func (c *Cluster) chooseTargetDataNodes(excludeZone string, excludeNodeSets []uint64, excludeHosts []string, replicaNum int, zoneName string) (hosts []string, peers []proto.Peer, err error) {
	...
	//   e.g.[zone0, zone1] -> [zone1, zone2] -> [zone2, zone0]
	//    -> [zone1, zone0] -> [zone2, zone1] -> [zone0, zone2]
	...
	if len(zones) == 3 {
		...
		case true:
			...
			zones = append(make([]*Zone, 0), zones[index], zones[(index + 1) % 3])
			c.lastPermutationsForZone = (c.lastPermutationsForZone + 1) % 6
		default:
			index := c.lastPermutationsForZone - 3
			zones = append(make([]*Zone, 0), zones[(index + 1) % 3], zones[index])
			c.lastPermutationsForZone = (c.lastPermutationsForZone + 1) % 6
		}
	}
...
func (c *Cluster) decommissionDataNode(dataNode *DataNode) (err error) {
	...
	for _, dp := range partitions {
		...
		go func(dp *DataPartition) {
			defer wg.Done()
			if err1 := c.decommissionDataPartition(dataNode.Addr, dp, getTargetAddressForDataPartitionDecommission, dataNodeOfflineErr); err1 != nil {
				errChannel <- err1
			}
		}(dp)
	}
...
func (c *Cluster) decommissionDataPartition(offlineAddr string, dp *DataPartition, chooseHostFunc func(c *Cluster, offlineAddr string, dp *DataPartition, excludeNodeSets []uint64, zoneName string) (oldAddr, newAddr string, err error), errMsg string) (err error) {
	var (
		oldAddr         string
		addAddr         string
	...
	if oldAddr, addAddr, err = chooseHostFunc(c, offlineAddr, dp, excludeNodeSets, vol.zoneName); err != nil {
		goto errHandler
	}
	if dpReplica, err = dp.getReplica(oldAddr); err != nil {
		goto errHandler
	}
	if err = c.removeDataReplica(dp, oldAddr, false); err != nil {
		return
	}
	...
	dp.isRecover = true
	c.putBadDataPartitionIDs(dpReplica, offlineAddr, dp.PartitionID)
	dp.RLock()
...
errHandler:
	msg = errMsg + fmt.Sprintf("clusterID[%v] partitionID:%v  on Node:%v  "+
		...
		c.Name, dp.PartitionID, offlineAddr, offlineAddr, err, dp.Hosts)
	if err != nil {
		Warn(c.Name, msg)
...
func (partition *DataPartition) RepairZone(vol *Vol, c *Cluster) (err error) {
	...
	if isNeedRebalance, err = partition.needToRebalanceZone(c, zoneList); err != nil {
		return
	}
	if !isNeedRebalance {
		return
	}
	if err = c.sendRepairDataPartitionTask(partition, BalanceDataZone, repairDataPartitionZoneErr); err != nil {
		log.LogErrorf("action[RepairZone] clusterID[%v] vol[%v] data partition[%v] err[%v]", c.Name, vol.Name, partition.PartitionID, err)
		return
	}
	return
}
var getTargetAddressForDataPartitionDecommission = func(c *Cluster, offlineAddr string, dp *DataPartition, excludeNodeSets []uint64, zoneName string) (oldAddr, newAddr string, err error){
	var (
		dataNode    *DataNode
		zone        *Zone
		zones       []string
	...
	if targetHosts, _, err = ns.getAvailDataNodeHosts(dp.Hosts, 1); err != nil {
		...
		if targetHosts, _, err = zone.getAvailDataNodeHosts(excludeNodeSets, dp.Hosts, 1); err != nil {
			...
			if targetHosts, _, err = c.chooseTargetDataNodesForDecommission(excludeZone, dp, dp.Hosts, 1, zoneName); err != nil {
				return
			}
		}
	}
...
func (c *Cluster) decommissionMetaNode(metaNode *MetaNode) (err error) {
	...
	for _, mp := range partitions {
		...
		go func(mp *MetaPartition) {
			defer wg.Done()
			if err1 := c.decommissionMetaPartition(metaNode.Addr, mp, getTargetAddressForMetaPartitionDecommission); err1 != nil {
				errChannel <- err1
			}
		}(mp)
	}
...
func (c *Cluster) setDpRecoverPoolSize(dpRecoverPool uint64) (err error) {
	oldDpPool := atomic.LoadUint64(&c.cfg.dataPartitionsRecoverPoolSize)
	atomic.StoreUint64(&c.cfg.dataPartitionsRecoverPoolSize, dpRecoverPool)

	if err = c.syncPutCluster(); err != nil {
		log.LogErrorf("action[setDpRecoverPoolSize] err[%v]", err)
		atomic.StoreUint64(&c.cfg.dataPartitionsRecoverPoolSize, oldDpPool)
		err = proto.ErrPersistenceByRaft
		return
	}
	return
...
func (c *Cluster) setMpRecoverPoolSize(mpRecoverPool uint64) (err error) {
	oldMpPool := atomic.LoadUint64(&c.cfg.metaPartitionsRecoverPoolSize)
	atomic.StoreUint64(&c.cfg.metaPartitionsRecoverPoolSize, mpRecoverPool)

	if err = c.syncPutCluster(); err != nil {
		log.LogErrorf("action[setMpRecoverPoolSize] err[%v]", err)
		atomic.StoreUint64(&c.cfg.metaPartitionsRecoverPoolSize, oldMpPool)
		err = proto.ErrPersistenceByRaft
		return
	}
	return
...
func (c *Cluster) sendRepairDataPartitionTask(dp *DataPartition, rType RepairType, errMsg string) (err error) {
	var repairTask *RepairTask
	repairTask = &RepairTask{
		RType: rType,
		Pid: dp.PartitionID,
	...
	default:
		err = fmt.Errorf("dpRepairChan has been full, errMsg:[%v]", errMsg)
	}
	return
}
