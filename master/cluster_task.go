...
func (c *Cluster) decommissionMetaPartition(nodeAddr string, mp *MetaPartition, chooseHostFunc func(c *Cluster, nodeAddr string, mp *MetaPartition, oldHosts []string, excludeNodeSets []uint64, zoneName string) (oldAddr, addAddr string, err error)) (err error) {
	var (
		addAddr         string
	...
	if nodeAddr, addAddr, err = chooseHostFunc(c, nodeAddr, mp, oldHosts, excludeNodeSets, vol.zoneName); err != nil {
		goto errHandler
	}
